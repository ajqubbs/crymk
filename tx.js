...
var rule = {
    ...
    class_name: 'SVIP4K🏆️电影&SVIP4K🏆️电视剧&SVIP4K🏆️综艺&SVIP4K🏆️动漫&SVIP4K🏆️少儿&SVIP4K🏆️纪录片',
    ...
    一级: $js.toString(() => {
        ...
        if (/get_playsource/.test(input)) {
            try {
                ...
                log("解析get_playsource出错:" + e.message)
            }
        } else {
            try {
                let json = JSON.parse(html);
                ...
                if (video_lists && video_lists.length === 1) {
                ...
                } else if (video_lists && video_lists.length > 1) {
                        ...
                        try {
                            ...
                            if (QZOutputJson && QZOutputJson.results) {
                                QZOutputJson.results.forEach(function(it1) {
                                    it1 = it1.fields;
                                    if (it1 && it1.title && !it1.title.includes("预告") && !it1.title.includes("花絮")) {
                            ...
                            log("解析union.video.qq.com出错:" + e.message)
                        }
                    })
                }
            } catch (e) {
                log("解析播放源出错:" + e.message)
            }
        }
        
        // 组装播放列表
        if (d.length > 0) {
            result.vod_play_from = "至尊4K独家线路";
            result.vod_play_url = d.map(function(it) {
    ...
    搜索: $js.toString(() => {
        ...
        function isMainContent(title) {
            ...
            return !nonMainContentKeywords.some(k => title.includes(k));
        }
